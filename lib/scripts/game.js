        ...
        for (let x = 0; x < layoutX; x++) {
            ...
            if (layout[x][y]) {
                if (colours[x][y]) {
                    ...
                    //         game.innerHTML += "<div class='tile' tabindex='0' style='background-color: "+colours[x][y]+"; grid-row: "+(y+1)+"; grid-column: "+(x+1)+";'><span class='tile-label light'>"+y+","+x+"</span><h1 class='tile-label light'>"+pos[y][x]+"</h1></div>";
                    //     } else {
                    //         game.innerHTML += "<div class='tile' tabindex='0' style='background-color: "+colours[x][y]+"; grid-row: "+(y+1)+"; grid-column: "+(x+1)+";'><span class='tile-label light hidden'>"+y+","+x+"</span><h1 class='tile-label light'>"+pos[y][x]+"</h1></div>";
                    //     }
                    // } else if (brightness(colours[x][y]) <= fish("#bright").value) {
                    //     if (fish("#debug").innerText == "on") {
                            game.innerHTML += "<div class='tile' tabindex='0' style='background-color: "+colours[x][y]+"; grid-row: "+(y+1)+"; grid-column: "+(x+1)+";'><span class='tile-label dark'>"+y+","+x+"</span><h1 class='tile-label dark'>"+pos[y][x]+"</h1></div>";
                    //     } else {
                    //         game.innerHTML += "<div class='tile' tabindex='0' style='background-color: "+colours[x][y]+"; grid-row: "+(y+1)+"; grid-column: "+(x+1)+";'><span class='tile-label dark hidden'>"+y+","+x+"</span><h1 class='tile-label dark'>"+pos[y][x]+"</h1></div>";
                    //     }
                    // }
                } else {
                    // if (fish("#debug").innerText == "on") {
                    //     game.innerHTML += "<div class='tile blank-tile' style='grid-row: "+(y+1)+"; grid-column: "+(x+1)+";'><span class='tile-label'>"+y+","+x+"</span><h1 class='tile-label'>"+pos[y][x]+"</h1></div>";
                    // } else {
                        game.innerHTML += "<div class='tile blank-tile' style='grid-row: "+(y+1)+"; grid-column: "+(x+1)+";'><span class='tile-label hidden'>"+y+","+x+"</span><h1 class='tile-label hidden'>"+pos[y][x]+"</h1></div>";
                    // }
                }   
            }
        }
...
function slide(tile,x,y) {
    if (pos[blankRow-2] != undefined && tile == pos[blankRow-2][blankCol-1]) {
        i = pos[x][y];
        pos[x][y] = pos[blankRow-1][blankCol-1];
        pos[blankRow-1][blankCol-1] = i;

        drawGame();
    } else if (pos[blankRow-1] != undefined && tile == pos[blankRow-1][blankCol]) {
        pos[x][y] = pos[blankRow-1][blankCol-1];
        i = pos[x][y];
        pos[x][y] = pos[blankRow-1][blankCol-1];
        pos[blankRow-1][blankCol-1] = i;
        ...
        drawGame();
    } else if (pos[blankRow-1] != undefined && tile == pos[blankRow-1][blankCol-2]) {
        i = pos[x][y];
        pos[x][y] = pos[blankRow-1][blankCol-1];
        pos[blankRow-1][blankCol-1] = i;

        drawGame();
    } else if (pos[blankRow] != undefined && tile == pos[blankRow][blankCol-1]) {
        i = pos[x][y];
        pos[x][y] = pos[blankRow-1][blankCol-1];
        pos[blankRow-1][blankCol-1] = i;
